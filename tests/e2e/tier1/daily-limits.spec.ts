...
function hasExLimitRule(rules: any[]): boolean {
  return rules.some(
    (r) => r.id >= LIMIT_RULE_ID_BASE && r.condition.urlFilter.includes('example.com'),
  );
}
...
test.describe('Tier 1: Restrictions & Limits', () => {
  test('an always-block restriction persists and blocks every visit', async ({
    context,
    extensionPage,
  }) => {
    ...
    await expect
      .poll(async () => hasExBlockRule(await getDynamicRules(extensionPage)), { timeout: 5000 })
      .toBe(true);
  ...
  test('a limit restriction persists; under the cap it is allowed and makes no block rule', async ({
    context,
    extensionPage,
  }) => {
    await addBlockedSite(extensionPage, { ...EX, mode: 'limit', dailyLimitSeconds: 3600 });
    await setStorage(extensionPage, {
      trackingData: { visits: {}, time: { [`example.com:${todayKey()}`]: 60 } },
    });
    ...
    expect(hasExBlockRule(rules), 'limit-mode site must never get an always-block rule').toBe(
      false,
    );
  ...
  test('a limit restriction over its cap blocks with reason=limit', async ({
    context,
    extensionPage,
  }) => {
    await addBlockedSite(extensionPage, { ...EX, mode: 'limit', dailyLimitSeconds: 3600 });
    await setStorage(extensionPage, {
      trackingData: { visits: {}, time: { [`example.com:${todayKey()}`]: 3700 } },
    });
    await checkLimits(extensionPage);

    await expect
      .poll(async () => hasExLimitRule(await getDynamicRules(extensionPage)), { timeout: 5000 })
      .toBe(true);
  ...
  test('toggling Always ⇄ Limit moves the rules between ranges', async ({ extensionPage }) => {
    ...
    await setStorage(extensionPage, {
      trackingData: { visits: {}, time: { [`example.com:${todayKey()}`]: 0 } },
    });
  ...
  test('the limit block page renders the DAILY LIMIT banner with site + usage', async ({
    context,
    extensionPage,
    extensionId,
  }) => {
    await addBlockedSite(extensionPage, { ...EX, mode: 'limit', dailyLimitSeconds: 3600 });
    await setStorage(extensionPage, {
      trackingData: { visits: {}, time: { [`example.com:${todayKey()}`]: 4200 } },
    }); // 1h10m
    await checkLimits(extensionPage);

    const page = await context.newPage();
    await page.goto(
      `chrome-extension://${extensionId}/blocked/blocked.html?site=example.com&reason=limit`,
      {
