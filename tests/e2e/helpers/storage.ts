...
export async function seedLimitRestriction(
  page: Page,
  opts: { id: string; label?: string; domains: string[]; pathOnly?: boolean; capSeconds: number; usedSeconds?: number }
): Promise<void> {
  await page.evaluate(async (o) => {
    const cur = (await chrome.storage.local.get('blockedSites')).blockedSites || [];
    const blocked = cur.filter((s: any) => s.id !== o.id);
    blocked.push({ id: o.id, label: o.label || o.id, domains: o.domains, builtin: false, pathOnly: !!o.pathOnly, mode: 'limit', dailyLimitSeconds: o.capSeconds });
    const td = (await chrome.storage.local.get('trackingData')).trackingData || { visits: {}, time: {} };
