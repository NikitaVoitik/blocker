...
test.describe('Tier 3: Real-Time Limit Accrual', () => {
  ...
  for (const c of CASES) {
    test(`blocks reddit after ~${c.label} of real focused browsing`, async ({
      context,
      extensionPage,
    }) => {
      const L = c.limitSec;
      ...
      while (waited < deadline) {
        ...
        if (!midChecked && waited >= L * 0.3 && waited <= L * 0.7) {
          ...
          expect(page.url(), `reddit still reachable @ ${waited}s real`).not.toContain(
            'blocked/blocked.html',
          );
        }

        if (res.overLimit.includes('reddit')) {
          blocked = true;
          break;
        }
      }

      expect(blocked, `reddit should cross its ${c.label} limit within ${deadline}s of focus`).toBe(
        true,
      );
      ...
      expect(finalData.reddit.time, 'recorded focus time reached the limit').toBeGreaterThanOrEqual(
        L,
      );
