...
import { setSiteRestriction, addBlockedSite, checkLimits, getDynamicRules } from '../helpers/messaging';
...
test.describe('Tier 3: Daily Limit Churn', () => {
  test('rapid set/clear toggling via SET_SITE_RESTRICTION never orphans limit rules', async ({ extensionPage }) => {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      for (const s of chosen) {
        await addBlockedSite(extensionPage, { id: s.id, label: s.id, domains: s.domains, mode: 'limit', dailyLimitSeconds: 60 });
